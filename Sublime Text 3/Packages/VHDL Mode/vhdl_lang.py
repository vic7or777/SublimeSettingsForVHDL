...
class Port():
    ...
    def parse_str(self, port_str):
        ...
        if s:
            ...
            # self.rst_port_name = s.group('rst_port_name')
            # Sometimes the type has a trailing space.  Eliminating it.
            self.type = re.sub(r'\s*$', '', s.group('type'))  

            if self.name.lower() == 'ce':
                self.init_data = "'1'" 
            elif self.type.lower() == 'std_logic':
                self.init_data = "'0'"            
            else:
                self.init_data = "(others=>'0')" 

            if self.name.lower() == 'rst':
                self.rst_port_name = 1
            elif  self.name.lower() == 'reset':   
                self.rst_port_name = 2
    ...
    def print_as_signal(self):
        ...
        # line = 'signal {} : {} := \'0\' '.format(self.name, self.type)        
        
        line = 'signal {} : {}:= {}'.format(self.name, self.type, self.init_data)
...
class Interface():
    ...
    def resetprocess(self):
        '''
        This method returns the reset process instantiations
        '''
        lines = []  
        check_rst = 0
        if self.if_ports:
            ...
            if check_rst == 1:                
                lines.append("RESET_Proc : process")
                lines.append("begin")
                lines.append("reset <= '1','0' after 10*clk_period; wait;")
                lines.append("end process RESET_Proc;")         
                indent_vhdl(lines, 1)
                return '\n'.join(lines)
            elif check_rst == 2:
                lines.append("RST_Proc : process")
                lines.append("begin")
                lines.append("rst <= '1','0' after 10*clk_period; wait;")
                lines.append("end process RST_Proc;")         
                indent_vhdl(lines, 1)
                return '\n'.join(lines)
        else:
            return None
    ...
    def testdata(self):
        '''
        Generate Test data in techtbench in relation to ports types
        ''' 
        lines = []
        bus_index = ""       
        testindex = 0
        ...
        if self.if_ports:
            for port in self.if_ports:            
                if port.mode.lower() == 'in':                    
                    # if port.name == ('clk' or 'ce' or 'rst'):
                    if port.name.lower() == ('clk'):
                        # lines.append("not")
                        testindex +=1
                    elif port.name.lower() == ('ce'):
                        # lines.append("not")
                        testindex +=1
                    ...
                    # if ('clk' or 'ce' or 'rst') in port.name :
                    #     testindex +=1
                    else:
                        lines.append("Process begin") 
                        if port.type.lower() == 'std_logic':
                            lines.append("{} <= '0'; wait for 1*clk_period;".format(port.name))   
                            lines.append("{} <= '1'; wait for 1*clk_period;".format(port.name))
                        else:                            
                            bus_index = re.search(r'\d+', port.type)
                            max_data = pow(2, int(bus_index.group(0))) - 1
                            lines.append("For i in 0 to {} loop".format(max_data))
                            lines.append("{} <= conv_std_logic_vector(i, {}+1);".format(port.name, bus_index.group(0)))
                            lines.append("wait for 1*clk_period;") 
        ...
        if self.if_ports:            
            ...
            if is_clk:            
                ...
                if is_rst and is_ce:
                    ...
                    lines.append("")
                    lines.append("end if; -- ce")


                lines.append("  end if;--clk")
            lines.append("end process;")
            # lines.append(str(testindex))
